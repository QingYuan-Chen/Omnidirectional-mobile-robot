...
- `[待实现]` 实现不依赖 HAL 的通用速度 PI + 前馈，包含条件积分、限幅和目标斜坡。
- `[待实现]` 根据每轮 30720 计数将编码器增量换算为轮速 RPM。
- `[待实现]` 增加适合 10 ms 控制周期的实测转速滤波。
- `[待实现]` 在 `controlTask` 接入 MA 编码器 -> 速度 PI + 前馈 -> 电机输出链路，并预留位置外环目标接口。
- `[待实现]` 实现 500 ms 命令超时、受控停车和停稳后 Brake。
- `[待实现]` 为 PI、前馈和换算函数增加 host 端单元测试。
...
- 首个闭环验证对象：MA 电机。
- 当前硬件没有电流 ADC 反馈，M2 不实现伪电流环；电机内环采用速度 PI + 前馈，可预留位置外环接口。
