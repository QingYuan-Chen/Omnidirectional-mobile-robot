...
- M2 G1 使用 TIM6 或 TIM7 更新事件产生候选 1 kHz 控制节拍。修改 `.ioc` 前先检查定时器、触发链、NVIC 优先级和 CubeMX 再生成边界。
...
- 当前 `ROBOT_CONFIG_CONTROL_PERIOD_MS=10` 仅描述现有占位行为；G1 完成后由明确命名的控制采样周期配置替代，不再混用 RTOS 延时周期和控制采样周期。
