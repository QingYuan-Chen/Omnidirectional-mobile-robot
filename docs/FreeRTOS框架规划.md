...
- 调试端口由 `app_debug_uart_config.h` 集中选择。2026-07-21 起，M2 阶段 3 板测镜像使用无线 DAPLink 虚拟串口对应的 USART1 PA9/PA10，230400 baud、8N1；USART2 PD5/PD6 恢复为树莓派正式 X-Protocol/ROS2 通信口，不得再同时承载人类 ASCII 调试协议。UART4 PC10/PC11 保留为旧调试硬件通道但当前不选用。命令为大写 ASCII：`ARM <seq>`、`PWM <seq> <value>`、`STOP <seq>`、`ESTOP`、`STATUS`；LF 结束，可在 LF 前带一个 CR。LF 前最多保存 64 B，第 65 B 触发行溢出并丢弃到下一次 LF。
