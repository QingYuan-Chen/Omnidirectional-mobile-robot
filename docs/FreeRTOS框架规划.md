...
| `controlTask` | 10 ms | High | 1536 B | 读取四路编码器、保持安全制动，后续承接运动学和 PID |
...
6. Control、Comm、IMU 使用事件标志上报心跳；Safety 每 100 ms 清点一次，连续 3 个窗口缺失即锁存故障。IMU 数据失效或陈旧会立即锁存故障并进入 Coast。
