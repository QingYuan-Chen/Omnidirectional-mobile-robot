...
static void TestValidCommands(void)
{
  ...
  assert(Feed(&protocol, "CAPTURE START\n", &command) == APP_COMM_FEED_COMMAND);
  assert(command.type == APP_COMM_COMMAND_CAPTURE_START);
  assert(Feed(&protocol, "CAPTURE STOP\n", &command) == APP_COMM_FEED_COMMAND);
  assert(command.type == APP_COMM_COMMAND_CAPTURE_STOP);
  assert(Feed(&protocol, "CAPTURE EXPORT\n", &command) == APP_COMM_FEED_COMMAND);
