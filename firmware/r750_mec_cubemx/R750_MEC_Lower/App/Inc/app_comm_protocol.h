 ...
 * 只表示语法与序号候选有效；ARM、PWM、STOP 必须在命令队列入队成功后再提交序号，
 * 从而允许队满时使用原序号重试。ESTOP 与 STATUS 不参与有序运动命令序列。
 */
...
/*
 * 提交一条已经成功进入电机命令队列的 ARM、PWM 或 STOP 序号。
 * 返回 true 表示序号已成为新的防重放基线；参数无效、命令类型不适用或序号不再更新时
 * 返回 false。不得在队列入队前调用，否则队满会造成合法重试被当作旧命令拒绝。
 */
