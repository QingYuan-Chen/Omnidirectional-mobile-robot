...
$schedule = @(New-G2FirstMotionSchedule `
    -Direction $Direction `
    -PeakPwm $PeakPwm `
...
$manifest = [ordered]@{
    ...
    safety_checks = [ordered]@{
        motor_power_disconnected_during_plan_generation = $true
        motor_power_connected_and_voltage_verified = $false
        polarity_and_common_ground_verified = $false
    ...
    notes = @(
        'This directory only prepares a command plan and does not open a serial port.',
